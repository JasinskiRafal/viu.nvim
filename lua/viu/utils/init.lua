...
local get_image_width_height = function(file)
  ...
  -- default to window height (for svgs, usually)
  local _height = vim.api.nvim_win_get_height(0)
...
M.calculate_ascii_width_height = function(buf_id, buf_path, opts)
  ...
  local width = math.floor(img_width * scale)
  local height = math.floor(img_height * scale)

  local horizntal_padding = math.floor((win_width - width) / 2)
  local vertical_padding = math.floor((win_height - height) / 2)

  return width, height, horizntal_padding, vertical_padding
end
